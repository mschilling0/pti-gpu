...
class ZeKernelCollector {
 public:  // Interface
  ...
  static void PrintKernelsTable(const ZeKernelInfoMap& kernel_info_map) {
    ...
    for (auto& value : sorted_list) {
      ...
      std::cerr << std::setw(max_name_length) << function << "," << std::setw(kCallsLength)
                << call_count << "," << std::setw(kSimdLength) << simd_width << ","
                << std::setw(kTimeLength) << duration << "," << std::setw(kPercentLength)
                << std::setprecision(2) << std::fixed << percent_duration << ","
  ...
  const ZeKernelIntervalList& GetKernelIntervalList() const { return kernel_interval_list_; }
 ...
 private:  // Implementation
  ...
  void AddKernelCommand(ze_command_list_handle_t command_list, const ZeKernelCommand* command) {
  ...
  void ProcessCallEvent(ze_event_handle_t event) {
  ...
  void ProcessCallKernel(const ZeKernelCommand* call) {
    PTI_ASSERT(call != nullptr);
    ...
    start_ns = start * static_cast<uint64_t>(NSEC_IN_SEC) / freq;
    if (start < end) {
      end_ns = end * static_cast<uint64_t>(NSEC_IN_SEC) / freq;
    } else {  // 32-bit timer overflow
      PTI_ASSERT(start < (1ULL << 32));
      end_ns = ((1ULL << 32) + end) * static_cast<uint64_t>(NSEC_IN_SEC) / freq;
    }
  ...
  void AddCommandList(ze_command_list_handle_t command_list, ze_context_handle_t context,
                      ze_device_handle_t device, bool immediate) {
    PTI_ASSERT(command_list != nullptr);
    PTI_ASSERT(context != nullptr);
    const std::lock_guard<std::mutex> lock(lock_);
    PTI_ASSERT(command_list_map_.count(command_list) == 0);
    command_list_map_[command_list] = {std::vector<const ZeKernelCommand*>(), context, device,
                                       immediate};
  }
 ...
 private:  // Callbacks
  static void OnEnterEventPoolCreate(ze_event_pool_create_params_t* params, ze_result_t result,
  ...
  static void OnEnterEventDestroy(ze_event_destroy_params_t* params, ze_result_t result,
    ...
    if (*(params->phEvent) != nullptr) {
      ZeKernelCollector* collector = reinterpret_cast<ZeKernelCollector*>(global_data);
      PTI_ASSERT(collector != nullptr);
      collector->ProcessCallEvent(*(params->phEvent));
    }
  }
  ...
  static void CreateEvent(ze_context_handle_t context, ze_event_pool_handle_t& event_pool,
                          ze_event_handle_t& event) {
    PTI_ASSERT(context != nullptr);
    ze_result_t status = ZE_RESULT_SUCCESS;
  ...
  static void OnEnterCommandListAppendLaunchKernel(
      ...
      void** instance_data) {
    OnEnterKernelAppend(utils::ze::GetKernelName(*(params->phKernel)),
                        utils::ze::GetKernelMaxSubgroupSize(*(params->phKernel)),
                        *(params->phSignalEvent), *(params->phCommandList), global_data,
                        instance_data);
  }

  static void OnExitKernelAppend(ze_command_list_handle_t command_list, void* global_data,
                                 void** instance_data, ze_result_t result) {
    PTI_ASSERT(command_list != nullptr);
  ...
  static void OnExitCommandListAppendLaunchKernel(
    ...
    OnExitKernelAppend(*params->phCommandList, global_data, instance_data, result);
  }
  ...
  static void OnExitCommandListCreate(ze_command_list_create_params_t* params, ze_result_t result,
    ...
    if (result == ZE_RESULT_SUCCESS) {
      ...
      ZeKernelCollector* collector = reinterpret_cast<ZeKernelCollector*>(global_data);
      PTI_ASSERT(collector != nullptr);
      collector->AddCommandList(**(params->pphCommandList), *(params->phContext),
